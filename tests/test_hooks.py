...
class TestAutoRegisterUnmappedSubagent:
    """Review Fourteen: general-purpose sub-agents skip SubagentStart.

    PreToolUse/PostToolUse events from such sub-agents arrive carrying a
    raw Claude hex ID that has no ``claude_agent_id`` mapping.  The hook
    must auto-register them as children of the session root so lock
    attribution, change notifications, and file ownership land on the
    right place in the hierarchy — not on a ghost top-level agent.
    """

    def test_auto_register_unmapped_raw_id_links_to_session_parent(self, hook_cwd):
        session_id = "sess-12345678"
        raw_claude_id = "aa67631390a989432"  # exact ID from Review Fourteen

        handle_session_start(_make_event("SessionStart", cwd=hook_cwd, session_id=session_id))
        # No SubagentStart — simulate general-purpose sub-agent that skipped it
        handle_pre_write(_make_event(
            "PreToolUse", tool_name="Write", cwd=hook_cwd, session_id=session_id,
            tool_input={"file_path": "/tmp/test_auto_reg.py"},
            subagent_id=raw_claude_id,
        ))
        ...
        engine = _get_engine(hook_cwd)
        try:
            session_parent = _session_agent_id(session_id)
            agents = engine.list_agents(active_only=False)["agents"]
            # Exactly one agent carries the raw Claude ID mapping
            mapped = [a for a in agents if a.get("claude_agent_id") == raw_claude_id]
            assert len(mapped) == 1, (
                f"Expected one auto-registered agent for {raw_claude_id}, "
                f"got {[a['agent_id'] for a in mapped]}"
            )
            # And it is linked to the session parent, not a detached root
            child = mapped[0]
            assert child["parent_id"] == session_parent
            assert child["agent_id"].startswith(f"{session_parent}.auto.")
            # Raw hex never leaks in as its own agent_id
            assert raw_claude_id not in [a["agent_id"] for a in agents]
        finally:
            engine.close()

    def test_auto_register_file_ownership_to_child(self, hook_cwd):
        """Review Fourteen P2: file ownership landed on parent instead of child.

        After auto-registration, a PostToolUse Write should claim
        file_ownership for the child's hub.cc.*.auto.<hex> ID, not the
        session root.
        """
        session_id = "sess-12345678"
        raw_claude_id = "afdff684e563fa1be"  # Review Fourteen Agent E

        handle_session_start(_make_event("SessionStart", cwd=hook_cwd, session_id=session_id))
        handle_post_write(_make_event(
            "PostToolUse", tool_name="Write", cwd=hook_cwd, session_id=session_id,
            tool_input={"file_path": "/tmp/test_auto_owner.py"},
            subagent_id=raw_claude_id,
        ))
        ...
        try:
            ...
            with engine._connect() as conn:
                row = conn.execute(
                    ...
                    "WHERE document_path LIKE '%test_auto_owner%'"
                ).fetchone()
                assert row is not None
                owner = row["assigned_agent_id"]
                assert owner != session_parent, (
                    "file_ownership attributed to session root, not the sub-agent"
                )
                assert owner.startswith(f"{session_parent}.auto.")
        finally:
            engine.close()

    def test_auto_register_idempotent_across_calls(self, hook_cwd):
        """Two tool calls from the same unmapped sub-agent share one registry entry."""
        session_id = "sess-12345678"
        raw_claude_id = "ba1122334455667ef"

        handle_session_start(_make_event("SessionStart", cwd=hook_cwd, session_id=session_id))
        handle_pre_write(_make_event(
            "PreToolUse", tool_name="Write", cwd=hook_cwd, session_id=session_id,
            tool_input={"file_path": "/tmp/test_idemp_a.py"},
            subagent_id=raw_claude_id,
        ))
        handle_post_write(_make_event(
            "PostToolUse", tool_name="Write", cwd=hook_cwd, session_id=session_id,
            tool_input={"file_path": "/tmp/test_idemp_b.py"},
            subagent_id=raw_claude_id,
        ))
        ...
        try:
            ...
            mapped = [a for a in agents if a.get("claude_agent_id") == raw_claude_id]
            assert len(mapped) == 1, (
                f"Second tool call duplicated the auto-registered agent: "
                f"{[a['agent_id'] for a in mapped]}"
            )
        finally:
            engine.close()

    def test_auto_register_prevents_parallel_overwrite(self, hook_cwd):
        """Review Fourteen P0: two sub-agents writing the same file must conflict.

        Before the fix both writes landed on the session root and never
        contended.  After the fix each sub-agent holds its own
        hub.cc.*.auto.<hex> ID, so the second PreToolUse is denied.
        """
        session_id = "sess-12345678"
        raw_a = "aa67631390a989432"
        raw_b = "afdff684e563fa1be"

        handle_session_start(_make_event("SessionStart", cwd=hook_cwd, session_id=session_id))
        result_a = handle_pre_write(_make_event(
            "PreToolUse", tool_name="Write", cwd=hook_cwd, session_id=session_id,
            tool_input={"file_path": "/tmp/test_parallel_write.md"},
            subagent_id=raw_a,
        ))
        result_b = handle_pre_write(_make_event(
            "PreToolUse", tool_name="Write", cwd=hook_cwd, session_id=session_id,
            tool_input={"file_path": "/tmp/test_parallel_write.md"},
            subagent_id=raw_b,
        ))
        assert result_a["hookSpecificOutput"]["permissionDecision"] == "allow"
        assert result_b is not None
        assert result_b["hookSpecificOutput"]["permissionDecision"] == "deny"
        assert "locked by" in result_b["hookSpecificOutput"]["permissionDecisionReason"]

    def test_auto_register_skips_hub_prefixed_ids(self, hook_cwd):
        """An event carrying a hub.cc.* ID must not be reshaped into an auto child."""
        session_id = "sess-12345678"

        handle_session_start(_make_event("SessionStart", cwd=hook_cwd, session_id=session_id))
        from coordinationhub.hooks.claude_code import _get_engine, _resolve_agent_id

        engine = _get_engine(hook_cwd)
        try:
            event = {
                "subagent_id": "hub.cc.explicit.child",
                "session_id": session_id,
            }
            resolved = _resolve_agent_id(event, engine=engine, auto_register=True)
            assert resolved == "hub.cc.explicit.child"
            # Nothing auto-registered
            agents = engine.list_agents(active_only=False)["agents"]
            auto = [a for a in agents if ".auto." in a["agent_id"]]
            assert auto == []
        finally:
            engine.close()
