"""Declarative coordination graph loader and validator for CoordinationHub.

Supports both YAML (via ruamel.yaml) and JSON coordination spec files.
Falls back gracefully if YAML library is unavailable.

Exports:
- load_graph, find_graph_spec, validate_graph, CoordinationGraph
- set_graph, get_graph, clear_graph (singleton)
- load_coordination_spec_from_disk (used by core.py on engine startup)
- validate_graph_tool (MCP tool implementation)

Schema:
{
  "agents": [{"id", "role", "model", "responsibilities": []}],
  "handoffs": [{"from", "to", "condition"}],
  "escalation": {"max_retries": int, "fallback": str},
  "assessment": {"metrics": [str]}
}

Zero third-party dependencies in this module.
"""
...
def find_graph_spec(project_root: Path | None) -> Path | None:
    """Look for coordination_spec.yaml then coordination_spec.json at project root."""
...
def load_coordination_spec_from_disk(
    ...
    Returns dict with loaded/paths/agent_count/agents keys on success,
